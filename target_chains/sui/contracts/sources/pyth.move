module pyth::pyth {
    ...
    const E_DATA_SOURCE_EMITTER_ADDRESS_AND_CHAIN_IDS_DIFFERENT_LENGTHS: u64 =
        0;
        ...
        setup::init_and_share_state(
            ...
            data_source::new(
                ...
                external_address::new((
                    bytes32::from_bytes(governance_emitter_address),
                )),
            ),
            parse_data_sources(
    ...
    public fun create_price_feeds_using_accumulator(
        ...
        // decode the price info updates from the VAA payload (first check if it
        // is an accumulator or batch price update)
    ...
    /// Create and share new price feed objects if they don't already exist
    /// using batch price attestation.
    /// The name of the function is kept as is to remain backward compatible
    public fun create_price_feeds(
        pyth_state: &mut PythState,
        // These vaas have been verified and consumed, so we don't have to worry
        // about
    ...
    // create_and_share_price_feeds_using_verified_price_infos is a private
    // function used by
    ...
    fun create_and_share_price_feeds_using_verified_price_infos(
        ...
        while (!vector::is_empty(&price_infos)) {
            ...
            if (
                ...
                // Create and share newly created Sui PriceInfoObject containing
                // a price feed,
                // and then register a copy of its ID with State.
                let new_price_info_object = price_info::new_price_info_object(
                    cur_price_info,
                    ctx,
                );
    ...
    // verified_vaa is the verified version of the VAA encoded within the
    // accumulator_message
    ...
    /// Update a singular Pyth PriceInfoObject (containing a price feed) with
    /// the
    /// price data in the authenticated price infos vector (a vector of
    /// PriceInfo objects).
    ///
    /// For more information on the end-to-end process for updating a price
    /// feed, please see the README.
    ///
    /// The given fee must contain a sufficient number of coins to pay the
    /// update fee for the given vaas.
    /// The update fee amount can be queried by calling get_update_fee(&vaas).
    ///
    /// Please read more information about the update fee here:
    /// https://docs.pyth.network/documentation/pythnet-price-feeds/on-demand#fees
    public fun update_single_price_feed(
        ...
        // On Sui, users get to choose which price feeds to update. They specify
        // a single price feed to
        // update at a time. We therefore charge the base fee for each such
        // individual update.
        // This is a departure from Eth, where users don't get to necessarily
        // choose.
        assert!(
            state::get_base_update_fee(pyth_state) <= coin::value(&fee),
            E_INSUFFICIENT_FEE,
        );
        ...
        // Find price update corresponding to PriceInfoObject within the array
        // of price_updates
        // and use it to update PriceInfoObject.
        let i = 0;
        let found = false;
        while (i < hot_potato_vector::length<PriceInfo>(&price_updates)) {
            let cur_price_info = hot_potato_vector::borrow<PriceInfo>(
                &price_updates,
                i,
            );
    ...
    fun has_same_price_identifier(
        ...
        let price_identifier_from_price_info = price_info::get_price_identifier(
            price_info,
        );
    ...
    public(friend) fun update_cache(
        ...
        let has_same_price_identifier = has_same_price_identifier(
            update,
            price_info_object,
        );
        assert!(
            has_same_price_identifier,
            E_UPDATE_AND_PRICE_INFO_OBJECT_MISMATCH,
        );
    ...
    /// Determine if the given price update is "fresh": we have nothing newer
    /// already cached for that
    ...
    fun is_fresh_update(
        ...
        let update_timestamp = price::get_timestamp(
            &price_feed::get_price(price_feed),
        );
        ...
        let cached_timestamp = price::get_timestamp(
            &price_feed::get_price(cached_price_feed),
        );
    ...
    /// Get the latest available price cached for the given price identifier, if
    /// that price is
    ...
    ///
    /// Important: Pyth uses an on-demand update model, where consumers need to
    /// update the
    /// cached prices before using them. Please read more about this at
    /// https://docs.pyth.network/documentation/pythnet-price-feeds/on-demand.
    /// get_price() is likely to abort unless you call update_price_feeds() to
    /// update the cached price
    /// beforehand, as the cached prices may be older than the stale price
    /// threshold.
    ///
    ...
    /// useful for any particular application. Users of this function should
    /// check
    ...
    /// Get the stale price threshold: the amount of time after which a cached
    /// price
    /// is considered stale and no longer returned by
    /// get_price()/get_ema_price().
...
module pyth::pyth_tests {
    ...
    fun ACCUMULATOR_TESTS_DATA_SOURCE(): vector<DataSource> {
        vector[
            data_source::new(
                1,
                external_address::new(
                    bytes32::from_bytes(ACCUMULATOR_TESTS_EMITTER_ADDRESS),
                ),
            ),
        ]
    }
    ...
    fun get_verified_test_vaas(
        ...
        while (i < vector::length(&test_vaas_)) {
            ...
            let verified_vaa = vaa::parse_and_verify(
                worm_state,
                cur_test_vaa,
                clock,
            );
    ...
    // get_verified_vaa_from_accumulator_message parses the accumulator message
    // up until the vaa, then
    ...
    fun get_verified_vaa_from_accumulator_message(
        ...
        let vaa = deserialize::deserialize_vector(
            &mut cursor,
            (vaa_size as u64),
        );
    ...
    fun check_price_feeds_cached(
        expected: &vector<PriceInfo>,
        actual: &vector<PriceInfoObject>,
    ) {
        // Check that we can retrieve the correct current price and ema price
        // for each price feed
        let i = 0;
        while (i < vector::length(expected)) {
            let price_feed = price_info::get_price_feed(
                vector::borrow(expected, i),
            );
    ...
    fun test_get_update_fee() {
        let (scenario, test_coins, _clock) = setup_test(
    ...
    fun test_create_price_feeds_corrupt_vaa() {
        let (scenario, test_coins, clock) = setup_test(
            500, /* stale_price_threshold */
            23, /* governance emitter chain */
            x"5d1f252d5de865279b00c84bce362774c2804294ed53299bc4a0389a5defef92",
            vector[],
        ...
        let verified_vaas = vector[
            vaa::parse_and_verify(&worm_state, corrupt_vaa, &clock),
        ];
    ...
    fun test_create_price_feeds_invalid_data_source() {
        // Initialize the contract with some valid data sources, excluding our
        // test VAA's source
    ...
    public fun data_sources_for_test_vaa(): vector<DataSource> {
        ...
        vector<DataSource>[
            ...
            data_source::new(
                17,
                external_address::new(
                    bytes32::new(ACCUMULATOR_TESTS_EMITTER_ADDRESS),
                ),
            ),
        ]
    }
    ...
    #[test]
    // test_create_and_update_price_feeds_with_batch_attestation_success tests
    // the creation and updating of price
    ...
    fun test_create_and_update_price_feeds_with_batch_attestation_success() {
        ...
        // Affirm that 4 objects, which correspond to the 4 new price info
        // objects
        ...
        // Create vector of price info objects (Sui objects with key ability and
        // living in global store),
        // which contain the price feeds we want to update. Note that these can
        // be passed into
        // update_price_feeds in any order!
        // et price_info_object_vec = vector[price_info_object_1,
        // price_info_object_2, price_info_object_3, price_info_object_4];
        ...
        assert!(
            price_info::get_balance(&price_info_object_1)==DEFAULT_BASE_UPDATE_FEE,
            0,
        );
    ...
    #[
        expected_failure(
            abort_code = pyth::accumulator::E_INVALID_ACCUMULATOR_PAYLOAD,
        ),
    ]
    ...
    fun test_create_and_update_multiple_price_feeds_with_accumulator_success() {
        ...
        // Affirm that 3 objects, which correspond to the 3 new price info
        // objects
        // containing the price feeds were created and shared.
        let effects = test_scenario::next_tx(&mut scenario, DEPLOYER);
        let shared_ids = test_scenario::shared(&effects);
        ...
        let idx = 0;
        let expected_price_infos = accumulator_test_3_to_price_info(
            0, /*offset argument*/
        );
        ...
        while (idx < 3) {
            ...
            assert!(
                price_feeds_equal(
                    &price_info,
                    vector::borrow(&expected_price_infos, idx),
                ),
                0,
            );
    ...
    fun test_update_cache() {
        ...
        // These updates are price infos that correspond to the ones in
        // TEST_VAAS.
        ...
        // Check that TEST_VAAS was indeed used to instantiate the price feeds
        // correctly,
        // by confirming that the info in updates is contained in
        // price_info_object_vec.
    ...
    fun test_update_cache_old_update() {
        ...
        // Hardcode the price identifier, price, and ema_price for
        // price_info_object_1, because
        // it's easier than unwrapping price_info_object_1 and getting the
        // quantities via getters.
        ...
        // Attempt to update the price with an update older than the current
        // cached one.
        ...
        let current_price_feed = price_info::get_price_feed(
            &current_price_info,
        );
        ...
        // Confirm that no price update occurred when we tried to update cache
        // with an
    ...
    // pyth accumulator tests (included in this file instead of
    // pyth_accumulator.move to avoid dependency cycle - as we need
    // pyth_tests::setup_test)
    #[test]
    ...
    fun test_parse_and_verify_accumulator_updates_with_extra_bytes_at_end_of_message() {
        ...
        // append some extra garbage bytes at the end of the accumulator
        ...
        vector::append(
            &mut test_accumulator_3_msgs_modified,
            x"1234123412341234",
        );
    ...
    // accumulator_test_3_to_price_info gets the data encoded within
    // TEST_ACCUMULATOR_3_MSGS
    ...
    // accumulator_test_1_to_price_info gets the data encoded within
    // TEST_ACCUMULATOR_SINGLE_FEED
    ...
    public fun take_wormhole_and_pyth_states(
        scenario: &Scenario,
    ): (PythState, WormState) {
